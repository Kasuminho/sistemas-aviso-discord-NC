...
export async function execute(interaction) {
  ...
  if (!memberData) {
    const emptyEmbed = new EmbedBuilder()
      ...
      .setDescription(`Olá <@${interaction.user.id}>! Você ainda não registrou seus status no bot.\n\nUse o comando \`/registrar-status\` anexando os prints da tela e informando seu nome no jogo para ficar elegível aos sorteios!`)
      .setTimestamp();
  ...
  const embed = new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: '⚡ PC Total (Poder)', value: `**${pcTotal}** (Mínimo: ${cutoff.minPC})`, inline: true },
    ...
    .setTimestamp()
    .setFooter({ text: 'NC Bot • Consulta Privada' });

  if (eligible) {
    embed.addFields({
      name: '🎉 Elegibilidade para Sorteios',
      value: '🟢 **VOCÊ ESTÁ 100% ELEGÍVEL PARA OS SORTEIOS DA SEMANA!**\nTodos os critérios de presença, desenvolvimento (PC) e atualização de status foram cumpridos.',
      inline: false
    });
  } else {
